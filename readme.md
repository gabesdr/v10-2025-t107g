# Verkefni 10, Vefforitun 107G "Free-For-All" verkefni.

## Verkefnalýsing
Fyrir verkefni 10 ég ákvaði að búa til "Random Fact Generator".

Kóðinn er frekar einfalt, hún notar `factGen.js` til að sækja eða búa til staðreyndir úr `Useless Facts API`.

Samkvæmt lýsningunni í `vef1-2025-v10` var sett upp `SCSS(SASS)`, `Stylelint` og `eslint`.

Hægt er að keyra command `npm run lint:js` og `npm run lint:css` til að kanna stöðuna og virkni á Javascript og CSS.

En auðvitað það þarf að sækja öll viðeigandi skrám með `npm install`.

Verkefnið er skrifuð á ensku.

Ferlið í notkun fyrir þessa verkefni er `Vite` ekki `Parcel`.

## Grunnur

- v10.2025.T107G (Main)
    - stylelint.config.mjs
    - readme.md
    - package.json
    - package-lock.json
    - index.html
    - eslint.config.js
    - .gitignore
    - src
        - api
            - factGen.js
        - main.js
        - styles.scss
    - public skrá sem inniheldur open-source myndir frá Iconify
        - octicon--logo-github-16.svg
        - octicon--mark-github-16.svg
    - fonts
        - Bricolage Grotesque

## Athugasemdir

N/A


> Útgáfa 1.0

## Útgáfusaga
| Útgáfa | Lýsing                     |
| ------ | -------------------------- |
| 1.0  | Fyrsta útgáfa verkefnisins |








